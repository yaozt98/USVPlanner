...
class Critic(nn.Module):
    def __init__(
        ...
        self.l1 = nn.Linear(
            lidar_feature_dim + position_state_dim + action_dim, hidden_dim)
        self.l4 = nn.Linear(
            lidar_feature_dim + position_state_dim + action_dim, hidden_dim)
    ...
    def Q1(self, lidar_state, position_state, action):
        lidar_feature = self.lidar_compress_net(lidar_state)
        state = torch.cat((lidar_feature, position_state), -1)
        sa = torch.cat([state, action], -1)
        q1 = F.relu(self.l1(sa))
...
class TD3(object):
    ...
    def train(self, replay_buffer, batch_size=100):
        ...
        with torch.no_grad():
            ...
            next_action = (
                self.actor_target(next_lidar_state, next_position_state) + noise
            ).clamp(-self.max_action, self.max_action)

            # Compute the target Q value
            target_Q1, target_Q2 = self.critic_target(
                next_lidar_state, next_position_state, next_action)
            target_Q = torch.min(target_Q1, target_Q2)
            target_Q = reward + not_done * self.discount * target_Q

        # Get current Q estimates
        current_Q1, current_Q2 = self.critic(lidar_state, position_state, action)
        ...
        if self.total_it % self.policy_freq == 0:
            ...
            actor_loss = -self.critic.Q1(
                ...
                self.actor(lidar_state.detach(), position_state.detach())).mean()
