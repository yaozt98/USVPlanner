"""
Function:
This is the training code of DDPG
...
def eval_policy(policy, eval_env, time_step=0, eval_episodes=50, save_directory=None,eval_config:dict=None):
    policy.eval_mode()
    avg_reward = 0.
    success_times = []
    success = 0
...
def create_eval_configs(eval_env):
    ...
    for i,num_episode in enumerate(num_episodes):
        for _ in range(num_episode):
            ...
            eval_env.reset()
            eval_config[f"env_{count}"] = eval_env.episode_data() # save eval config
            count += 1
...
if __name__ == "__main__":
    ...
    parser.add_argument("--discomfort_distance", type=float, default=40)
    ...
    if args.test:
        success_rate, collision_rate, avg_nav_time, avg_return = eval_policy(policy, eval_env, eval_episodes=50, save_directory=file_prefix + '/evaluation_episodes')
    ...
    for t in range(1, int(args.max_timesteps) + 1):
        ...
        if t < args.start_timesteps:
            ...
            with torch.no_grad():
                _, _ = policy.select_action(lidar_state, position_state)
        else:
            with torch.no_grad():
